...
static __INLINE uint8_t wireless_reg_read(uint8_t reg)
{
	...
	//Initialize value to receive data 
	uint8_t value; 
	
	uint8_t tx_data[2];
	uint8_t rx_data[2];
	
	//Clear the upper 3 bits, leave the bottom 5 for address
...
static __INLINE void wireless_set_tx_addr(uint8_t  *tx_addr)
{
	...
	tx_data[0] = NRF24L01_CMD_W_REGISTER | 0x10; 
	for (x=0; x<5; x++) {
		tx_data[x+1]=tx_addr[x];
	}

	
	wireless_CSN_low(); 
	
	//First byte is write command, next 5 data
...
{
	uint8_t myID[]      = { '3', '5', '3', '5', '3'};
	uint8_t remoteID[]  = { '3', '5', '3', '5', '4'};
